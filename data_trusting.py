...
def main():
    ...
    all_features = list(range(train_vectors.shape[1]))  # Changed to list for Python 3
    ...
    cv_preds = sklearn.model_selection.cross_val_predict(classifier, train_vectors, train_labels, cv=5)  # Updated from cross_validation to model_selection
    parzen.fit(train_vectors, cv_preds)
    sigmas = {
        'multi_polarity_electronics': {'neighbors': 0.75, 'svm': 10.0, 'tree': 0.5, 'logreg': 0.5, 'random_forest': 0.5, 'embforest': 0.75},
    ...
    for i in range(test_vectors.shape[0]):
        print(i)  # Changed print to use parentheses
        sys.stdout.flush()
    ...
    for untrustworthy in untrustworthy_rounds:
        t = test_vectors.copy()
        t[:, untrustworthy] = 0
        mistrust_idx = np.where(classifier.predict(t) != classifier.predict(test_vectors))[0]  # Changed np.argwhere to np.where for Python 3
        print(f'Number of suspect predictions {len(mistrust_idx)}')  # Changed to f-string
    ...
    print(f'Average number of flipped predictions: {np.mean(flipped_preds_size)} +- {np.std(flipped_preds_size)}')
    print('Precision:')
    for expl in explainer_names:
        mean_prec = np.mean(precision[expl])
        std_prec = np.std(precision[expl])
        pvalue = sp.stats.ttest_ind(precision[expl], precision['LIME'])[1].round(4)
        print(f'{expl} {mean_prec} +- {std_prec} pvalue {pvalue}')

    #print()
    print('Recall:')
    for expl in explainer_names:
        mean_rec = np.mean(recall[expl])
        std_rec = np.std(recall[expl])
        pvalue = sp.stats.ttest_ind(recall[expl], recall['LIME'])[1].round(4)
        print(f'{expl} {mean_rec} +- {std_rec} pvalue {pvalue}')

    #print()
    print('F1:')
    for expl in explainer_names:
        mean_f1 = np.mean(f1[expl])
        std_f1 = np.std(f1[expl])
        pvalue = sp.stats.ttest_ind(f1[expl], f1['LIME'])[1].round(4)
        print(f'{expl} {mean_f1} +- {std_f1} pvalue {pvalue}')
