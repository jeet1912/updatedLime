...
def get_random_indices(labels, class_, probability):
    nonzero = np.where(labels == class_)[0]  # Changed to np.where for Python 3
...
def add_corrupt_feature(feature_name, clean_train, clean_test, dirty_train,
    ...
    for class_ in set(train_labels):
        ...
        for i in indices:
            clean_train[i] += f' {fake_prefix}{feature_name}{fake_prefix}'  # Changed to f-string
        indices = get_random_indices(train_labels, class_, class_probs_dirty[class_])
        for i in indices:
            dirty_train[i] += f' {fake_prefix}{feature_name}{fake_prefix}'  # Changed to f-string
        indices = get_random_indices(test_labels, class_, class_probs_clean[class_])
        for i in indices:
            clean_test[i] += f' {fake_prefix}{feature_name}{fake_prefix}'  # Changed to f-string
...
def corrupt_dataset(independent_features, train_data, train_labels, test_data, test_labels):
    ...
    for probs, probs2, n in independent_features:
        for i in range(n):
            add_corrupt_feature(f'{idx}', clean_train, clean_test, dirty_train, train_labels, test_labels, probs, probs2)  # Changed to f-string
            idx += 1
...
def main():
    ...
    for Z in range(args.num_rounds):
        ...
        for expl in explainer_names:
            exps1[expl] = []
            exps2[expl] = []
        print(f'Round {Z}')  # Changed to f-string
        sys.stdout.flush()
        fake_features_z = [([.1, .2], [.1,.1], 10)]  # Removed commented out line for simplicity
        ...
        iteration = 0
        found_tree = True
        while abs(train_acc1 - train_acc2) > 0.001 or abs(test_acc1 - test_acc2) < 0.05:  # Changed abs to Python 3 function
            iteration += 1
            ...
            if iteration == 3000:
                ...
                break
        if not found_tree:
            print(f'skipping iteration {Z}')  # Changed to f-string
            continue
        print('done')
        print(f'Train acc1: {train_acc1}, Train acc2: {train_acc2}')  # Changed to f-string
                print(f'Test acc1: {test_acc1}, Test acc2: {test_acc2}')  # Changed to f-string
        sys.stdout.flush()
        ...
        with open(os.path.join(args.output_folder, f'comparing_{dataset}_{args.num_features}_{Z + args.start_id}.pickle'), 'wb') as f:  # Changed to 'wb' for binary mode in Python 3
            pickle.dump(out, f)
